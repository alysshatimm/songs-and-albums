...
import { deleteSong } from "./songDelete.js";
  ...
  albums.forEach((album) => {
    ...
    album.songs.forEach((songs) => {
      ...
      songs.songComments.forEach((songComment) => {
        ...
        fetch(`http://localhost:8080//api/song/${songs.id}/comments`, {
          method: "GET",
          mode: "cors",
          headers: {
      ...
      let songRatingText = document.getElementById(
        `song-average-stars${songs.id}`
      );
    ...
    let albumRatingText = document.getElementById(
      `album-average-rating${album.id}`
    );
